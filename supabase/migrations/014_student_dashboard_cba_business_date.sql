-- Keep the dashboard state aligned with lifecycle eligibility in the CBA business timezone.
    ...
    CASE
      ...
      WHEN EXISTS (
        ...
        WHERE status = 'completed'
          AND public.fn_cba_business_date(completed_at) = public.fn_cba_business_date(NOW())
      ) THEN 'completed'
      ELSE 'available'
    END,
